...
- The *ureABCDEFG* operon is consistently recovered on a single contig (5–29 kb span) in the three better-assembled MAGs (M1, S13, S16); limited assembly contiguity prevents identical verification in C22/S23/S26, but every MICP-complete MAG encodes every *ure* gene.
...
- The *ureABCDEFG* operon is recovered on a single contig in the three best-assembled MICP-complete MAGs (M1, S13, S16), is accompanied by at least one *cah* gene in every MICP-complete MAG, and shows no flanking mobile-element signatures or GC deviation — consistent with vertical inheritance within each lineage.
...
## Table 1 | Assembly quality and *ure–cah* cluster contiguity of the six MICP-complete ("MICP-complete") MAGs.
...
## Table 2 | Trait modules significantly enriched in MICP-complete MAGs after BH-FDR correction (q < 0.05).
See main text § 3.4; full statistics for all 38 modules in Supplementary Table S2.

## Table 3 | dbCAN class-level CAZyme enrichment in MICP-complete MAGs (HMMER-3, E < 1 × 10⁻¹⁵ for MICP-completees; DRAM `cazy_best_hit` for the remaining 105 MAGs).

| CAZy class | MICP-complete mean (per 10³ CDS) | Rest mean (per 10³ CDS) | Fold change | 95 % bootstrap CI | Permutation BH-q |
|---|---|---|---|---|---|
