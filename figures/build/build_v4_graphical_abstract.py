"""Graphical abstract (SUBMISSION_v4, 2026-09-23) - one composed page, 190 x 105 mm.

Derived from the 2026-09-09 page.  The input donut now shows the true sample origins of
the 111 MAGs (dairy-cattle manure, manured soil, soil; enrichment cultures of the
microcosms of Perez-Valera and Elhottova, 2025) instead of the former four "waste
streams", which were the binning tools.  The MGnify tile is replaced by the codeml M0
purifying-selection tile.
...
import _style as st
from _style import (HERO, REST, SPHINGO, PSEUDO, GREEN, GREY, TEXT, AXIS, LIGHT, ORIGIN,
                    HEROES, LINEAGE, hero_col, PROVENANCE)
from _micp_presence import presence

st.setup()
OUT = HERE / "figures_v4"
SUB = Path("/data/data/Upcycling/SUBMISSION_v4")
...
n_src = prov.origin.value_counts()
SRC_ORDER = ["Manure", "Manured soil", "Soil"]
N_MAG = int(n_src.sum())
...
m0 = pd.read_csv(SUPP / "Table_S19b_codeml_M0_summary.csv").set_index("gene")
omega_max = float(m0.loc[["ureA", "ureB", "ureC", "ureG"], "omega_M0"].max())
...
# ---- title band
T(W / 2, 6.5, "Cattle manure–soil microcosm MAGs yield two convergent lineages with a "
              "stably retained $\\it{ure}$–$\\it{cah}$ module", 10.5, ha="center",
  fontweight="bold")
...
T(27, ROW_T + 5.0, "111 enrichment-culture MAGs", 7.5, ha="center", fontweight="bold")
...
for s_ in SRC_ORDER:
    ...
    ax.add_patch(Wedge((cx, cy), r_out, a1, a0, width=r_out - r_in,
                       facecolor=ORIGIN[s_], edgecolor="white", lw=0.8, zorder=3))
...
for k, s_ in enumerate(SRC_ORDER):
    yk = ROW_T + 46.0 + k * 4.4
    xk = 12.0
    ax.add_patch(Circle((xk, yk), 1.4, facecolor=ORIGIN[s_], edgecolor="none", zorder=4))
    T(xk + 2.6, yk, f"{s_.lower()}  n = {n_src[s_]}", 6.5)
T(27, ROW_T + 59.0, "dairy-cow manure · soil microcosms · day 2–28", 6, ha="center", color=GREY)
...
T(87, oy + 8.6, "no recent mobilization in either lineage", 6.0, ha="center", color=AXIS)
...
tiles = [
    ...
    (f"ω ≤ {omega_max:.2f}", "codeml M0 on $\\it{ureA/B/C/G}$",
     "strong purifying selection", AXIS),
...
print(f"  {N_MAG} MAGs | all-8 {n_all8} | sites {n_site_match}/{n_site_cells} | fold {n_fold} | "
      f"Mrp {mrp_fold:.1f}x | omega max {omega_max:.3f} | ANI {ani_txt}")
